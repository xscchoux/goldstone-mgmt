...
class TestPlatformServer(XlateTestCase):
    ...
    async def test_create_circuit_pack(self):
        def test():
            ...
            data = self.conn.get(f"/org-openroadm-device:org-openroadm-device/circuit-packs[circuit-pack-name='{sys_comp_name}']")
            expected = {
                # user provisioned data
    ...
    async def test_create_circuit_pack_port(self):
        def test():
            ...
            data = self.conn.get(f"/org-openroadm-device:org-openroadm-device/circuit-packs")
