...
class TestTransponderServer(XlateTestCase):
    ...
    async def test_get_otsi_entries(self):
        def test():
            ...
            # data = self.con.get(f"/goldstone-transponder:modules/module[name='{piu_val}']/network-interface[name='0']/config/output-power")
            ...
            # or_val = "or_val2"
            # slot_val = 0
            # setup_otsi_connections(conn, piu_val, or_val, slot_val)

            # # Queries are done from operational mode
            # # sess.switch_datastore("operational")
            # data = conn.get_operational(f"/org-openroadm-device:org-openroadm-device/interface[name='{or_val}']/name")
            # self.assertEqual(data, or_val)
